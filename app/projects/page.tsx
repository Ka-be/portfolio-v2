	...
	return (
		<div className="h-[100dvh] w-[100dvw] overflow-hidden relative">
			...
			<div className="w-[calc(100%-calc(var(--frame-size)*1.8))] h-[calc(100dvh-calc(var(--frame-size)*2))] font-lexend flex items-center justify-center m-[var(--frame-size)]">
				...
				<div className="absolute bottom-0 right-0 w-4/5 md:w-1/3 h-[calc(100dvh-calc(var(--frame-size)*2))] flex flex-col justify-end overflow-hidden"
					style={{
				...
				>
					<ul className="overflow-y-auto max-h-full scrollbar-thin scrollbar-track-transparent scrollbar-thumb-foreground/20 hover:scrollbar-thumb-foreground/40 pr-4">
						{projects.map((project) => (
							...
							>
								<div className="mr-2">
									<ArrowDownLeft size={64} strokeWidth={0.8}/>
								</div>
								{project.title}
							</li>
						))}
					...
					{selectedProject && (
						<section className="flex flex-col justify-between w-full h-full">
							...
							<div className="flex flex-col w-3/4 h-full justify-end pb-4">
								...
								<p className="text-lg font-light mb-6 text-left">{selectedProject.description}</p>
								...
								<a href={selectedProject.link} className="text-sm text-foreground/80 hover:text-foreground">Voir le projet</a>
							</div>
						</section>
					)}
				</div>
