...
const TestCarousel = () => {
	...
	// Height de carousel a 500vh -> A baisser pour un scroll plus horizontal

	return (
		<div
			className="carousel min-h-screen bg-slate-500 flex items-start justify-center h-[500vh] p-0"
			ref={targetRef}
		>
			<div className="contentContainer h-[100vh] sticky top-0 flex items-center justify-start overflow-hidden bg-blue-500">
				<motion.div
					className="images flex py-0 pl-16 pr-16 space-x-16"
					style={{ x }}
				>
					{[1, 2, 3, 4, 5].map((item) => (
						<motion.div
							key={item}
							className="imageItem bg-gray-800 h-[100vh] w-[100vw] flex items-center justify-center text-4xl font-bold"
